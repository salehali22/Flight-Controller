...
volatile int8_t  g_init_result = -128;  /* Init not attempted yet */
volatile int8_t  g_config_result = -128;
volatile int16_t g_acc_x = 0, g_acc_y = 0, g_acc_z = 0;
...
BMP388_Data data;
BMP388_Status init_status;
BMP388_Status read_status;
HAL_StatusTypeDef i2c_ready;
...
int main(void)
{
  ...
  /* Initialize BMI270 (uploads config file) */
  g_init_result = bmi270_interface_init(&bmi270);

  if (g_init_result != BMI2_OK) {
      /*
       * Common error codes:
       *   BMI2_E_NULL_PTR       = -1
       *   BMI2_E_COM_FAIL       = -2
       *   BMI2_E_DEV_NOT_FOUND  = -3
       *   BMI2_E_CONFIG_LOAD    = -11
       *
       * Check g_init_result in Live Expressions
       */
      while (1) {
          HAL_Delay(100);
      }
  ...
  g_config_result = bmi270_configure_sensor(&bmi270);

  if (g_config_result != BMI2_OK) {
      while (1) {
          HAL_Delay(100);
      }
  }
  ...
  i2c_ready = HAL_I2C_IsDeviceReady(&hi2c2, (0x76 << 1), 3, 100);
  ...
  while (1)
  {
      ...
      if (bmi270_read_sensor_data(&bmi270, &ax, &ay, &az, &gx, &gy, &gz) == BMI2_OK)
      {
          g_acc_x = ax;
          g_acc_y = ay;
          g_acc_z = az;
      ...
      read_status = BMP388_ReadData(&bmp388, &data);
      HAL_Delay(200);
