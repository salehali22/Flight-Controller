...
void imu_mpu6050_loop() {
  ...
  if (now - sample_time >= SAMPLE_INTERVAL) {
    // Record processing start time to account for execution time
    ...
    // Only print if serial buffer has space (non-blocking check)
    // This prevents blocking when buffer is full, which would delay timing
    if (Serial.availableForWrite() > 0) {
      print_orientation();
    }

    // Account for processing time: set next sample time based on when we STARTED
    // plus the interval, not when we finished. This prevents drift accumulation.
    unsigned long processing_time = millis() - processing_start;
    sample_time += SAMPLE_INTERVAL;  // Additive timing to prevent drift

    // If processing took longer than interval, skip ahead to prevent backlog
